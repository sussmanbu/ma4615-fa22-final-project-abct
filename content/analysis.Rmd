...
ggplot(schooldata3, aes(x = Total.Score, y = State.Code)) + 
  geom_boxplot(x = Total.Score) + 
...
Lastly, we decided to use backward selection to drop any predictor variables that made our model less precise. Our model 8 is the best model here since it has the lowest Mallow's CP value, and contains all the income bracket variables, indicating that income is indeed a good predictor of SAT scores. 
```