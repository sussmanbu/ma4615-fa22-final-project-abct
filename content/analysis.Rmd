...
model2 <- lm(income.more.than.100k ~ total.score, schooldata)
summary(model2)
...
model3<- lm(income.less.than.20k ~ total.score, schooldata)
summary(model3)
...
```{r model 4}
ggplot(schooldata) + geom_point(aes(x = total.score, y = income.more.than.100k, col = state.code)) + labs(x = "SAT Score", y = "Income")
```
