...
```{r model 2}
schooldatacleaned <- schooldatacleaned %>% group_by(income.bracket) %>% transmute(avg.total.score = mean(total.score))
schooldatacleaned

ggplot(schooldatacleaned) + geom_point(aes(x = income.bracket, y = total.score, col = state.code))
```
