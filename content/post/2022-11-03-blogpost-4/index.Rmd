...
ggplot(data) + geom_point(aes(x = log(income.between.20.40k), y = log(total.score), col = state.code)) + labs(x = "Income", y = "SAT Score")
```
```{r plot 2}

data <- newdata

ggplot(data) + geom_point(aes(x = log(income.more.than.100k), y = log(total.score), col = state.code)) + labs(x = "Income", y = "SAT Score")
